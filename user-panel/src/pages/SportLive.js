...
export default function SportLive({ setPage, sportKey, title, icon }) {
  ...
  const handlePlaceBet = () => {
    ...
    const newBet = {
      ...
      matchDesc: selectedBet.matchDesc || `${title} Match`,
      stake: stake,
      return: estReturn
    };
  ...
  useEffect(() => { 
    setLoading(true);
    fetch(`http://localhost:5000/api/sports/${sportKey}/live`)
      .then(res => res.json())
      .then(data => {
      ...
      <div style={{ maxWidth: 1400, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16, flexWrap: "wrap", gap: 8 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div>
              <div style={{ fontSize: 18, fontWeight: 800 }}>{title} Center</div>
        ...
        <div style={{ marginBottom: 32 }}>
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            ...
            {loading ? (
              <div style={{ padding: 40, textAlign: "center", color: "var(--text-3)", background: "var(--navy-3)", borderRadius: 8, border: "1px solid var(--border)" }}>Loading {title}...</div>
            ) : matches.length === 0 ? (
                    ...
                    <div style={{ flex: 1, cursor: "pointer" }} onClick={() => setSelectedBet(null)}>
                      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
                        {m.status === "LIVE" ? (
                          ...
                          <span style={{ fontSize: 10, background: "var(--yellow)", color: "#000", padding: "2px 6px", borderRadius: 4, fontWeight: 800 }}>UPCOMING</span>
                        )}
