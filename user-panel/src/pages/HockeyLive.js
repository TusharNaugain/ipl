...
export default function HockeyLive({ setPage }) {
  ...
  const handlePlaceBet = () => {
    ...
    const newBet = {
      ...
      matchDesc: selectedBet.matchDesc || "Ice Hockey Match",
      stake: stake,
      return: estReturn
    };
  ...
  useEffect(() => { 
    fetch('http://localhost:5000/api/hockey/live')
      .then(res => res.json())
      .then(data => {
      ...
      <div style={{ maxWidth: 1400, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16, flexWrap: "wrap", gap: 8 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div>
              <div style={{ fontSize: 18, fontWeight: 800 }}>Ice Hockey Center</div>
              <div style={{ fontSize: 11, color: "var(--text-3)" }}>NHL & Global Leagues</div>
            </div>
            {matches.length > 0 && (
        ...
        <div style={{ marginBottom: 32 }}>
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            ...
            {loading ? (
              <div style={{ padding: 40, textAlign: "center", color: "var(--text-3)", background: "var(--navy-3)", borderRadius: 8, border: "1px solid var(--border)" }}>Loading Hockey...</div>
            ) : matches.length === 0 ? (
