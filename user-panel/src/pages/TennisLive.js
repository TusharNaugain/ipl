...
export default function TennisLive({ setPage }) {
  const [tennisMatches, setTennisMatches] = useState([]);
  const [loadingTennis, setLoadingTennis] = useState(true);
  ...
  const handlePlaceBet = () => {
    ...
    const newBet = {
      ...
      matchDesc: selectedBet.matchDesc || "Tennis Match",
      stake: stake,
      return: estReturn
    };
  ...
  useEffect(() => { 
    fetch('http://localhost:5000/api/tennis/live')
      .then(res => res.json())
      .then(data => {
      ...
      <div style={{ maxWidth: 1400, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16, flexWrap: "wrap", gap: 8 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div>
              <div style={{ fontSize: 18, fontWeight: 800 }}>Tennis Center</div>
        ...
        <div style={{ marginBottom: 32 }}>
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            ...
            {loadingTennis ? (
              <div style={{ padding: 40, textAlign: "center", color: "var(--text-3)", background: "var(--navy-3)", borderRadius: 8, border: "1px solid var(--border)" }}>Loading Tennis...</div>
            ) : tennisMatches.length === 0 ? (
              <div style={{ padding: 40, textAlign: "center", color: "var(--text-3)", background: "var(--navy-3)", borderRadius: 8, border: "1px solid var(--border)" }}>No tennis matches currently available.</div>
            ) : (
              tennisMatches.map((m, i) => (
                <div key={m.id || i} style={{ background: "var(--navy-3)", borderRadius: 8, border: "1px solid var(--border)", transition: "var(--transition)", overflow: "hidden" }}
                  ...
                  <div style={{ display: "flex", alignItems: "center", padding: "16px 20px" }}>
                    ...
                    {/* Tennis Odds Section (1 - 2, no draw) */}
                    <div style={{ display: "flex", gap: 32 }}>
                      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
                        <span style={{ fontSize: 11, fontWeight: 700, color: "var(--text-3)" }}>1 (Player A)</span>
                      ...
                      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
                        <span style={{ fontSize: 11, fontWeight: 700, color: "var(--text-3)" }}>2 (Player B)</span>
