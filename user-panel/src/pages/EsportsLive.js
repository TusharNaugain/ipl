...
export default function EsportsLive({ setPage }) {
  ...
  const handlePlaceBet = () => {
    ...
    const newBet = {
      ...
      matchDesc: selectedBet.matchDesc || "Esports Match",
      stake: stake,
      return: estReturn
    };
  ...
  useEffect(() => { 
    fetch('http://localhost:5000/api/esports/live')
      .then(res => res.json())
      .then(data => {
        if (data && Array.isArray(data)) setMatches(data);
        setLoading(false);
      })
      .catch(err => {
      ...
      <div style={{ maxWidth: 1400, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16, flexWrap: "wrap", gap: 8 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div>
              <div style={{ fontSize: 18, fontWeight: 800 }}>Esports Center</div>
              <div style={{ fontSize: 11, color: "var(--text-3)" }}>CS:GO, LoL, Dota 2 & More</div>
            </div>
            {matches.length > 0 && (
        ...
        <div style={{ marginBottom: 32 }}>
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            ...
            {loading ? (
              <div style={{ padding: 40, textAlign: "center", color: "var(--text-3)", background: "var(--navy-3)", borderRadius: 8, border: "1px solid var(--border)" }}>Loading Esports...</div>
            ) : matches.length === 0 ? (
              <div style={{ padding: 40, textAlign: "center", color: "var(--text-3)", background: "var(--navy-3)", borderRadius: 8, border: "1px solid var(--border)" }}>No matches currently available.</div>
            ) : (
              matches.map((m, i) => (
                  ...
                  <div style={{ display: "flex", alignItems: "center", padding: "16px 20px" }}>
                    ...
                    <div style={{ display: "flex", gap: 32 }}>
                      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
                        <span style={{ fontSize: 11, fontWeight: 700, color: "var(--text-3)" }}>1 (Team A)</span>
                      ...
                      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
                        <span style={{ fontSize: 11, fontWeight: 700, color: "var(--text-3)" }}>2 (Team B)</span>
