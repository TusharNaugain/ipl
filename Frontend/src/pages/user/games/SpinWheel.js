...
const SEGMENTS = [
  { label: "2x",  color: "#6c63ff", emoji: "💜" },
  { label: "0x",  color: "#ff6584", emoji: "❌" },
  { label: "3x",  color: "#43e97b", emoji: "💚" },
  { label: "0x",  color: "#ff6584", emoji: "❌" },
  { label: "5x",  color: "#ffc700", emoji: "💛" },
  { label: "0x",  color: "#ff6584", emoji: "❌" },
  { label: "1x",  color: "#63b3ff", emoji: "💙" },
  { label: "10x", color: "#ff9f43", emoji: "🔥" },
];
...
export default function SpinWheel({ setPage }) {
  ...
  const spin = async () => {
    ...
    const targetAngle = Math.random() * 360 + extraSpins;
    setRotation(prev => prev + targetAngle);

    setTimeout(async () => {
      try {
        const res = await api.post("/game/spin", { amount: parseInt(amount) });
        setResult(res);
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 28 }}>
        ...
        <div>
          <div style={{ fontSize: 24, fontWeight: 700 }}>🎰 Spin Wheel</div>
          <div style={{ fontSize: 13, color: "var(--text-3)" }}>Spin and win up to 10x your bet!</div>
        </div>
      </div>
      ...
      <div className="card" style={{ textAlign: "center" }}>
        ...
        <div style={{ display: "inline-block", position: "relative", marginBottom: 28 }}>
          <div style={{
            ...
            boxShadow: "0 0 0 6px var(--border), 0 8px 32px rgba(0,0,0,0.4)"
          }} />
          {/* Center */}
          <div style={{
            position: "absolute", top: "50%", left: "50%", transform: "translate(-50%,-50%)",
            width: 48, height: 48, borderRadius: "50%",
            background: "var(--bg-base)", border: "3px solid var(--border)",
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", justifyContent: "center", marginBottom: 24 }}>
          {[...new Set(SEGMENTS.map(s => s.label))].map(label => {
            ...
            return (
              <span key={label} className="badge" style={{ background: seg.color + "22", color: seg.color, border: `1px solid ${seg.color}44` }}>
                {seg.emoji} {label}
              </span>
            );
          })}
        ...
        {result && (
          <div style={{ marginBottom: 20, padding: "16px", background: "var(--bg-surface)", borderRadius: 12, border: "1px solid var(--border)" }}>
            <div style={{ fontSize: 24, fontWeight: 700, color: result.won ? "#43e97b" : "#ff6584" }}>
        ...
        <div className="form-group" style={{ maxWidth: 240, margin: "0 auto 20px" }}>
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 8, justifyContent: "center" }}>
            {[50, 100, 500].map(v => (
              <button key={v} className="btn" onClick={() => setAmount(v)} disabled={spinning}
                style={{ padding: "4px 10px", fontSize: 12, background: "var(--bg-hover)", border: "1px solid var(--border)", color: "var(--text-2)" }}>{v}</button>
            ))}
          </div>
        </div>
